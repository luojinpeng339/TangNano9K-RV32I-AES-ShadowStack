#!/usr/bin/env python3
"""Minimal two-pass assembler for OS-0C uart_line_monitor.S.
...
SOURCE = Path(__file__).with_name("uart_line_monitor.S")
OUTPUT = Path(__file__).with_name("uart_line_monitor.hex")
...
for pc, text in lines:
    ...
    elif mnemonic == "aes32esi":
        # aes32esi rd, rs1, rs2, bs ? ratified Zkne encoding.
        bs = imm(ops[3]); word = ((bs & 3) << 30) | (0b10001 << 25) | (REG[ops[2]] << 20) | (REG[ops[1]] << 15) | (0b000 << 12) | (REG[ops[0]] << 7) | 0x33
    elif mnemonic == "lw":
